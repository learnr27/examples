...
class IndexView(View):
    ...
    def get(self, request):
        ...
        # print(Course.objects.aggregate(Max('price'), Min('price'), Avg('price'), Sum('volume'))) # {'price__max': 296, 'price__min': 202, 'price__avg': 264.5556, 'volume__sum': 56731}

        # courses = Course.objects.values('teacher').annotate(title=GroupConcat('title', distinct=True,
        #                                                                   ordering='title ASC',
        #                                                                   separator='-'))
        # 报错 【GroupConcat does not allow distinct.】
        # for c in courses:
        #     print(c)
